 ...
 * 
 * In the @see PDServlet class a PDHibernateFactory instance is created
 * at the beginning of each request from the browser. The factory returns
 * a singleton hibernate session instance during the whole duration of the 
 * current request. Only when the request is going to finish is this session
 * commited and closed.
 * 
 * In case no hibernate/database interaction takes place at all during
 * that request, no session is created (and closed) either.
 */
